...
// 新しいフレームサイズ
const frameWidth = 5600;  // 8列 × 700px (7日 + Weekly)
const frameHeight = 4800; // 6週 × 800px

// 月間カレンダーグリッドの作成
- ヘッダー: 年月表示（例: "2025年 12月"）
- 曜日ヘッダー: 日本語表示（日、月、火、水、木、金、土、Weekly）
...
### 3. フック（ドロップ検知）の安定化
- **Math-based Calculation**: ヒットテスト（図形検索）から座標計算による日付判定に変更し、API遅延の影響を排除
- **リサイズ対応**: カレンダーフレームがリサイズされた場合でも正確に判定できるよう、ヘッダー高さなどを比率計算に変更
- **ポーリング最適化**: 250ms間隔での監視と、ドロップ時の即時反応（100msデバウンス）を実装
- **Rate Limit対策**: 不要なAPIコールを削減し、選択検知ロジックを軽量化
